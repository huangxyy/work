...
> ⚠️ **安全警告**: 以下密码仅适用于本地开发环境。生产环境部署时必须修改为强密码！

| 角色 | 账号 | 密码（开发环境） |
|------|------|------------------|
| 管理员 | admin | Dev@Pass2024! |
| 教师 | teacher01 | Dev@Pass2024! |
| 学生 | student01 | Dev@Pass2024! |
...
- **唯一性**: 不同服务使用不同密码
- **定期更换**: 建议每 90 天更换一次
- **生成工具**: 使用密码管理器或 `openssl rand -base64 24` 生成
...
开发环境使用 seed 脚本创建的测试账号，密码为 `Dev@Pass2024!`。

> ⚠️ **生产环境**: 设置 `SEED_USERS=false` 禁用自动创建测试账号。
