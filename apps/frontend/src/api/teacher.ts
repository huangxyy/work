...
export type TeacherBatchUploadResult = {
  ...
  matchResults?: Array<{
    ...
    analysisEn?: string;
    reason?: string;
    extractedName?: { zh: string; pinyin: string };
    thumbnailUrl?: string;
  }>;
};
...
export const createTeacherBatchSubmissions = async (payload: {
  ...
  if (payload.nameOverrides) {
    formData.append('nameOverrides', JSON.stringify(payload.nameOverrides));
  }
  if (payload.excludedFileKeys) {
    formData.append('excludedFileKeys', payload.excludedFileKeys);
  }
  if (payload.images) {
...
export const retrySkippedSubmission = async (
  homeworkId: string,
  fileKey: string,
  filename: string,
  studentName: string,
  ...
  const response = await api.post('/teacher/submissions/retry-skipped', {
    homeworkId,
    fileKey,
    filename,
    studentName,
    batchId, // Link to original BatchUpload
  });
