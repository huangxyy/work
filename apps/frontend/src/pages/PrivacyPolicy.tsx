      ...
      <main className="apple-legal-content">
        ...
        <section className="apple-legal-section">
          ...
          <ul className="apple-legal-list">
            ...
            <li>
              ...
              {isZh ? '：上传的作业图片、OCR 识别文本、批改结果' : ': Uploaded homework images, OCR-recognized text, grading results'}
            </li>
            <li>
              <strong>{isZh ? '使用记录' : 'Usage Records'}</strong>
              {isZh ? '：登录时间、操作日志、系统访问记录' : ': Login time, operation logs, system access records'}
            </li>
          </ul>
        </section>
        ...
        <section className="apple-legal-section">
          ...
          <ul className="apple-legal-list">
            ...
            <li>{isZh ? '生成学习报告和统计分析' : 'Generate learning reports and statistical analysis'}</li>
            <li>{isZh ? '改进 AI 模型和服务质量' : 'Improve AI models and service quality'}</li>
            <li>{isZh ? '保障系统安全和防止滥用' : 'Ensure system security and prevent abuse'}</li>
          </ul>
          <Typography.Paragraph>
            {isZh
              ? '我们承诺不会将您的个人数据出售给第三方。'
        ...
        <section className="apple-legal-section">
          <Typography.Title level={3}>
            {isZh ? '3. 数据存储与安全' : '3. Data Storage and Security'}
          </Typography.Title>
          <Typography.Paragraph>
            {isZh
          ...
          <ul className="apple-legal-list">
            ...
            <li>{isZh ? '基于角色的访问控制' : 'Role-based access control'}</li>
            <li>{isZh ? '操作日志审计' : 'Operation log auditing'}</li>
            <li>{isZh ? '定期安全检查' : 'Regular security audits'}</li>
          </ul>
        </section>
        ...
        <section className="apple-legal-section">
          ...
          <Typography.Paragraph>
            {isZh
              ? '本服务面向教育机构使用。如果您未满 18 岁，请在家长或监护人的指导下使用本服务，并在他们的同意下阅读本隐私政策。我们不会故意收集未满 14 岁儿童的个人信息，除非获得其家长或监护人的同意。'
        ...
        <section className="apple-legal-section">
          <Typography.Title level={3}>
            {isZh ? '8. Cookie 和本地存储' : '8. Cookies and Local Storage'}
          </Typography.Title>
          <Typography.Paragraph>
            {isZh
              ? '本服务使用以下本地存储技术：'
          ...
          <ul className="apple-legal-list">
            ...
            <li>
              ...
              {isZh ? '：存储临时会话数据' : ': Stores temporary session data'}
            </li>
          </ul>
          <Typography.Paragraph>
            {isZh
        ...
        <section className="apple-legal-section">
          ...
          <ul className="apple-legal-list">
            <li>{isZh ? '联系您所在学校的管理员' : 'Contact your school administrator'}</li>
